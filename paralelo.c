...
// ---------- Utilidades Básicas de Matrices ----------

// Lee la matriz desde el archivo "entrada.ent"
// Función no paralelisable por ser de lectura y no "thread-safe".
void leerEntrada(const char *nombreArchivo) {
    FILE *archivo = fopen(nombreArchivo, "r");
    if (!archivo) {
        fprintf(stderr, "Error: No se pudo abrir entrada.ent\n");
        exit(EXIT_FAILURE);
    }
...
void guardarSalidaInvalida() {
    FILE *salida = fopen("salida_paralela.sal", "w");
    fprintf(salida, "-1\n");
    fclose(salida);
}
...
void guardarSalida(char tipo, double matriz[MAX][MAX], int filas, int columnas) {
    FILE *salida = fopen("salida_paralela.sal", "w");
    if (!salida) {
        fprintf(stderr, "Error: No se pudo escribir salida_paralela.sal\n");
        exit(EXIT_FAILURE);
    }
    fprintf(salida, "%c\n", tipo);
    for (int i = 0; i < filas; i++) {
